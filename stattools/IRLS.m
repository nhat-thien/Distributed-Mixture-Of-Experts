function res = IRLS(X, Y, Winit, weights, max_iter, threshold, verbose)
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[n, K] = size(Y);
[~, q] = size(X);% q here is (q+1)

if nargin<7; verbose = 0;end
if nargin<6; threshold = 1e-5;end
if nargin<5; max_iter = 500;end % if there is no a specified max_iter
if nargin<4; weights = ones(n,1);end % observation weights
...
[piik_old, loglik_old] =multinomial_logistic(W_old,X,Y);
...
if verbose >= 1; fprintf(1,'IRLS : Iteration : %d "Log-lik": %f\n', iter, loglik_old);end
%% IRLS
...
%     end
    ...
    % or using:
    gw_old = (weights.*X)' * ( Y(:,1:end-1) - piik_old(:,1:end-1) );
...
%             end
    ...
    [piik, loglik] = multinomial_logistic(W,X,Y);
    ...
    converge1 = abs((loglik-loglik_old)/loglik_old) <= threshold;
    converge2 = abs(loglik-loglik_old) <= threshold;
...
if converge
    ...
    if verbose >= 0 % force silence, even without convergence
        fprintf('\n');
